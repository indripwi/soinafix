...
@section('content')
    <main class="main">
        <div class="container py-5">
            ...
            <form action="{{ route('pendaftaran.update', $pendaftaran->slug) }}" method="POST" enctype="multipart/form-data"
                class="row g-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" id="nama_pendaftar" name="nama_pendaftar"
                        value="{{ $pendaftaran->nama_pendaftar }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="nik" class="form-label">NIK</label>
                    <input type="text" class="form-control" id="nik" name="nik" value="{{ $pendaftaran->nik }}"
                        required>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="nomor_telepon" class="form-label">Nomor Telepon</label>
                    <input type="text" class="form-control" id="nomor_telepon" name="nomor_telepon"
                        value="{{ $pendaftaran->nomor_telepon }}" required>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <label for="nik" class="form-label">Alamat Lengkap</label>
                    <input type="text" class="form-control" id="alamat" name="alamat"
                        value="{{ $pendaftaran->alamat }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="sekolah" class="form-label">Asal Sekolah</label>
                    <input type="text" class="form-control" id="sekolah" name="sekolah"
                        value="{{ $pendaftaran->sekolah }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="kelas" class="form-label">Kelas</label>
                    <input type="text" class="form-control" id="kelas" name="kelas"
                        value="{{ $pendaftaran->kelas }}" required>
                </div>
               @foreach ([
    ...
    <div class="col-md-6 mb-3">
        ...
        @if ($pendaftaran->$data)
            <div class="mt-2">
                <a href="{{ asset('storage/' . $pendaftaran->$data) }}" target="_blank"
                    class="btn btn-sm btn-outline-primary">
                    <i class="fas fa-eye"></i> Lihat File Lama
                </a>
                <a href="{{ route('pendaftar.download', ['file' => $pendaftaran->$data]) }}"
